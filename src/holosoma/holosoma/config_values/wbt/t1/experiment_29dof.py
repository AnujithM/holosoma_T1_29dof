"""Whole Body Tracking experiment presets for the T1-29dof robot."""
...
from holosoma.config_values.wbt.t1.command_29dof import t1_29dof_wbt_command
from holosoma.config_values.wbt.t1.termination_29dof import t1_29dof_wbt_termination
from holosoma.config_values.wbt.t1.reward_29dof import t1_29dof_wbt_reward, t1_29dof_wbt_fast_sac_reward

from holosoma.config_values.loco.t1.experiment import t1_29dof, t1_29dof_fast_sac

# Valid body names for T1 29-DOF robot (from URDF/MuJoCo XML)
VALID_T1_29DOF_BODY_NAMES = [
    "Trunk",
    "H1", "H2",
    ...
    "Shank_Left", "Ankle_Cross_Left", "left_foot_link", "left_foot_contact_point",
    "Hip_Pitch_Right", "Hip_Roll_Right", "Hip_Yaw_Right",
    "Shank_Right", "Ankle_Cross_Right", "right_foot_link", "right_foot_contact_point",
]

# Joint names for T1 29-DOF robot control
T1_29DOF_JOINT_NAMES = [
...
# 1. PPO-based Whole Body Tracking for T1 29-DOF robot
t1_29dof_wbt = ExperimentConfig(
    training=TrainingConfig(
        project="WholeBodyTracking",
        name="t1_29dof_wbt_manager",
        num_envs=4096,
    ),
    ...
    robot=replace(
        ...
        body_names=VALID_T1_29DOF_BODY_NAMES,
    ),
    terrain=terrain.terrain_locomotion_plane,
    observation=g1_29dof_wbt_observation,
    action=replace(
        action.g1_29dof_joint_pos,
        params={"asset_name": "robot", "joint_names": T1_29DOF_JOINT_NAMES},
    ),
    termination=t1_29dof_wbt_termination,
    command=t1_29dof_wbt_command,
    reward=t1_29dof_wbt_reward,
...
# 2. FastSAC-based Whole Body Tracking for T1 29-DOF robot
t1_29dof_wbt_fast_sac = ExperimentConfig(
    training=TrainingConfig(
        project="WholeBodyTracking",
        name="t1_29dof_wbt_fast_sac_manager",
        num_envs=1024,
    ),
    env_class="holosoma.envs.wbt.wbt_manager.WholeBodyTrackingManager",
    algo=replace(
        ...
        config=replace(
            ...
            num_learning_iterations=800000,
            save_interval=10000,
            v_max=20.0,
            v_min=-20.0,
            gamma=0.99,
    ...
    termination=t1_29dof_wbt_termination,
    command=t1_29dof_wbt_command,
    reward=t1_29dof_wbt_fast_sac_reward,
    randomization=g1_29dof_wbt_randomization,
    curriculum=curriculum.g1_29dof_wbt_curriculum,
    nightly=NightlyConfig(
        iterations=200000,
...
__all__ = ["t1_29dof_wbt", "t1_29dof_wbt_fast_sac"]
